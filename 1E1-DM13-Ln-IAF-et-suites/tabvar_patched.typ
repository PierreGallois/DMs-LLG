...
#let largeur_correspondance_entete_taille = 10.65
#let taille_texte_col1 = 10.65mm
#let hauteur_ligne = 1.5
#let marge_fleche = 0.5
#let marge_nbr_gauche = 0.3
#let marge_nbr_droite = 0.25
#let marge_nbr_bas = 0.3
#let unknown = 0.15
#let largeur_double_barre = 0.07
...
#let _coord-fleche(i, j, indef, proch_indef, ligne, coordX, coordY) = { // Caclule les points de départ et d’arrivé des flèches 
  ...
  let h = measure(ligne.at(j).last()).height.mm() /(largeur_correspondance_entete_taille)
  let l = measure(ligne.at(j).last()).width.mm() /(largeur_correspondance_entete_taille)

  let n = measure(ligne.at(indice_proch_ele).at(
    if proch_indef {if ligne.at(indice_proch_ele).at(1) == "||"{2} else {3}}
    else {1}
    )).height.mm() /(largeur_correspondance_entete_taille)
  let d = measure(ligne.at(indice_proch_ele).at(
    if proch_indef {if ligne.at(indice_proch_ele).at(1) == "||"{2} else {3}}
    else {1}
  )).width.mm() /(largeur_correspondance_entete_taille)
  ...
  let (x,y) = ( //les coordonées de l'element
    coordX.at(j).at(0)
    + if indef and j != 0 { l/2 + marge_nbr_droite} else if indef and j == 0{marge_nbr_bas} else {0}
    ,
    coordY.at(i).at(0) 
    + if ligne.at(j).at(
      ...
      coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(ligne.at(j).at(
        if ligne.at(j).len() > 2 { 
          if ligne.at(j).at(1) == "||"{
            2
          } else {
      ...
      )).height.mm() /(2 * largeur_correspondance_entete_taille)
  ...
  let (a,b) = ( //les coordonées du prochain element
    ...
    + if proch_indef and proch_indef != 0 {- d/2 - marge_nbr_droite} else {0},

    coordY.at(i).at(0) 
    + if ligne.at(indice_proch_ele).first() == top {
      coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(ligne.at(indice_proch_ele).at(
  ...
  context {
    cetz.canvas({
      ...
      let largeur_permiere_colonne = calc.max( //La largeur de la première colonne
        ...
        calc.max(
          ..for i in range(0, init.label.len()){
            (measure(init.label.at(i).at(0)).width.mm() / largeur_correspondance_entete_taille + 1,)
          }
        )
      )
      ...
      let hauteur_permiere_ligne = calc.max(
        1,
        measure(init.variable).height.mm() / (largeur_correspondance_entete_taille),
        ..for i in domain {
          (measure(i).height.mm() / (largeur_correspondance_entete_taille) + 1 ,)
        }
      )
      ...
      let decalage_domaine = largeur_permiere_colonne+marge_nbr_gauche + calc.max( // la distance entre les valeurs du domaine puis la largeur du talbeau de signe
        measure(domain.at(0)).width.mm() / (2 * largeur_correspondance_entete_taille),
        ..for i in range(0, contents.len()){
          if init.label.at(i).last() == "Variation" {
            (measure(contents.at(i).at(0).last()).width.mm() / (2 * largeur_correspondance_entete_taille),)
          } else {(0,)}
        }
      )  
      ...
      for i in range(0, domain.len() - 1){ //les éléments du domaine
        coordX.at(i) = (decalage_domaine, calc.max(
          measure(domain.at(i)).width.mm() / ( largeur_correspondance_entete_taille),
          ..for j in range(0, contents.len()){
            if init.label.at(j).last() == "Variation" {
              let oui = {
                if contents.at(j).at(i).len() > 2 {
                  calc.max(
                    measure(contents.at(j).at(i).last()).width.mm() /(largeur_correspondance_entete_taille),
        ...
        let prochX1 = {
          calc.max(
            measure(domain.at(i + 1)).width.mm() / (largeur_correspondance_entete_taille),
            ..for j in range(0, contents.len()){
              if init.label.at(j).last() == "Variation" {
                let oui = {
                  if contents.at(j).at(i + 1).len() > 2 {
                    calc.max(
                      measure(contents.at(j).at(i + 1).last()).width.mm() /(largeur_correspondance_entete_taille),
        ...
        if i == 0{
          ...
          content(
            (decalage_domaine, -hauteur_permiere_ligne / 2),
            box(width: 3 * taille_texte_col1, align(center, domain.at(i)))
          )
        }
      ...
      content( // le dernier éléments du domaine
        (decalage_domaine, -hauteur_permiere_ligne / 2),
        box(width: 3 * taille_texte_col1, align(center, domain.at(domain.len() - 1)))
      )
      coordX.at(domain.len() - 1) = (
        decalage_domaine, 
        calc.max(
          measure(domain.at(domain.len() - 1)).width.mm() / ( largeur_correspondance_entete_taille),
          ..for j in range(0, contents.len()){
            if init.label.at(j).last() == "Variation" {
              let oui = {
                if contents.at(j).at(domain.len() - 1).len() > 2 {
                  calc.max(
                    measure(contents.at(j).at(domain.len() - 1).last()).width.mm() /(largeur_correspondance_entete_taille),
      ...
      decalage_domaine = decalage_domaine + coordX.at(domain.len() - 1).at(1)/2 + marge_nbr_gauche
      ...
      for i in range(-0, init.label.len()){ //le texte de la première colonne
        let hauteur_case = calc.max(
          1,
          measure(init.label.at(i).at(0)).height.mm() / largeur_correspondance_entete_taille,
          ..for j in range(contents.at(i).len()) {
            let ele = contents.at(i).at(j)
            if init.label.at(i).last() == "Sign" {
              if type(ele) == array and ele.len() > 2 {
                (measure(ele.last()).height.mm() / largeur_correspondance_entete_taille,)
              } else if  type(ele) != array and ele != "||" {
                (measure(ele).height.mm() / largeur_correspondance_entete_taille,)
              } else {
                (1,)
              }
               ...
               let oui = {
                if contents.at(i).at(j).len() > 2 {
                  calc.max(
                    measure(contents.at(i).at(j).last()).height.mm() /(largeur_correspondance_entete_taille),
        ...
        content(
          (largeur_permiere_colonne / 2, - hauteur_total - hauteur_case / 2 - hauteur_ligne/2 ),
          box(width: largeur_permiere_colonne * taille_texte_col1, align(center ,init.label.at(i).at(0)))
        )
        ...
        coordY.at(i) = (- hauteur_total - hauteur_case / 2 - hauteur_ligne/2, hauteur_case + hauteur_ligne)

        hauteur_total = hauteur_total + hauteur_case + hauteur_ligne
      }
      ...
      for i in range(0, init.label.len()){ // Début des différents tableaux
        ...
        if init.label.at(i).at(1) == "Sign"{
            ...
            if type(contents.at(i).at(j)) == array and contents.at(i).at(j).len() >= 2 { // le signe si le signe n'est pas vide mais à une bar spécial
              ...
              if j != 0{
                ...
                if contents.at(i).at(j).at(0) == "||"{
                  line(
                    (coordX.at(j).at(0) - largeur_double_barre, coordY.at(i).at(0) - coordY.at(i).at(1)/2),
                    (coordX.at(j).at(0) - largeur_double_barre, coordY.at(i).at(0) + coordY.at(i).at(1)/2),
                  )
                  line(
                    (coordX.at(j).at(0) + largeur_double_barre, coordY.at(i).at(0) - coordY.at(i).at(1)/2),
          ...
          if contents.at(i).at(contents.at(i).len() - 1) == "||" { // Si bar indéfinie à la fin
            line(
              (decalage_domaine - unknown, coordY.at(i).at(0) - coordY.at(i).at(1)/2),
              (decalage_domaine - unknown, coordY.at(i).at(0) + coordY.at(i).at(1)/2),
            )
          } else {
            ...
            if type(contents.at(i).at(indice - 1)) == array and contents.at(i).at(indice - 1).len() >= 2 { // si le signe n'est pas vide
                ...
                if contents.at(i).at(indice - 1).at(0) == "||"{
                  line(
                    (coordX.at(indice - 1).at(0) - largeur_double_barre, coordY.at(i).at(0) - coordY.at(i).at(1)/2),
                    (coordX.at(indice - 1).at(0) - largeur_double_barre, coordY.at(i).at(0) + coordY.at(i).at(1)/2),
                  )
                  line(
                    (coordX.at(indice - 1).at(0) + largeur_double_barre, coordY.at(i).at(0) - coordY.at(i).at(1)/2),
        ...
        if init.label.at(i).last() == "Variation" {
            ...
            if contents.at(i).at(j).len() == 2{
              ...
              let (x,y) = ( //les coordonées de l'element
                  ...
                  + if contents.at(i).at(j).first() == top {
                    coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(contents.at(i).at(j).last()).height.mm() /(2 * largeur_correspondance_entete_taille)
              ...
              line(
                (
                  coordX.at(j).at(0) - largeur_double_barre,
              ...
              content( // éléments à gauche de la bar
                (
                  coordX.at(j).at(0) - measure(element_gauche).width.mm()/ (2 *largeur_correspondance_entete_taille) - marge_nbr_droite, 
                  coordY.at(i).at(0)
                  + if contents.at(i).at(j).first() == top {
                    coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(element_gauche).height.mm() /(2 * largeur_correspondance_entete_taille)
                  } else if contents.at(i).at(j).first() == bottom {
                    - coordY.at(i).at(1) / 2 + marge_nbr_bas + measure(element_gauche).height.mm() /(2 * largeur_correspondance_entete_taille)
                  } else {0}
                ),
              ...
              content( // éléments à droite de la bar
                (
                  coordX.at(j).at(0) + measure(element_droite).width.mm()/ (2 *largeur_correspondance_entete_taille) + marge_nbr_droite, 
                  coordY.at(i).at(0)
                  + if contents.at(i).at(j).at(if contents.at(i).at(j).at(1) == "||" {0} else {1}) == top {
                    coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(element_droite).height.mm() /(2 * largeur_correspondance_entete_taille)
          ...
          if der_ele.len() > 2 { //Si il y a une bar indef
            ...
            content(
              (
                coordX.at(indice_der_ele).at(0)- marge_nbr_droite, 
                coordY.at(i).at(0)
                + if contents.at(i).at(indice_der_ele).first() == top {
                  coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(contents.at(i).at(indice_der_ele).last()).height.mm() /(2 * largeur_correspondance_entete_taille)
          ...
          if pr_ele.len() > 2 {
            ...
            content(
              (
                coordX.at(0).at(0) + marge_nbr_bas, 
                coordY.at(i).at(0)
                + if contents.at(i).at(0).at(if contents.at(i).at(0).at(1) == "||" {0} else {1}) == top {
                  coordY.at(i).at(1) / 2 - marge_nbr_bas - measure(contents.at(i).at(0).last()).height.mm() /(2 * largeur_correspondance_entete_taille)
